...
final4 <- final3 |>
  dplyr::select(grouping_desc, catch_count_hbll, catch_count_dog, offset, offset_hbll, offset_dog, UTM.lat, UTM.lon) |>
  distinct()
...
# # model length and depth
#
# hbll2 <- hbll |>
#    mutate(depth_bin = ifelse(grepl("Depth Stratum 2", grouping_desc), "depth_56-100m",
#                                       ifelse(grepl("Depth Stratum 3", grouping_desc), "depth_111-165m",
#                                              ifelse(grepl("Depth Stratum 4", grouping_desc), "depth_166-220m",
#                                              ifelse(grepl("Depth Stratum 5", grouping_desc), "depth_>220m", NA
#                                       ))))) |>
#   group_by(length_bin, depth_bin, fishing_event_id, grouping_desc, year, catch_count_hbll, latitude, longitude) |>
#   reframe(count_hbll_ld = n(), survey_abbrev_hbll = "HBLL_13/0", length_bin = length_bin, depth_bin = depth_bin, offset_hbll = mean(offset_hbll)) |>
#   drop_na(length_bin) |>
#   dplyr::select(-fishing_event_id, -year) |>
#   ungroup() |>
#   distinct()
...
#                                           ifelse(grepl("Depth Stratum 5", grouping_desc), "depth_>220m", NA
#                                           ))))) |>
#   group_by(length_bin, depth_bin, fishing_event_id, grouping_desc, year, catch_count_dog) |>
#   reframe(count_dog_ld = n(), survey_abbrev_dog = "Dog_14/0", length_bin = length_bin, depth_bin = depth_bin, offset_dog = mean(offset_dog)) |>
#   drop_na(length_bin) |>
#   dplyr::select(-fishing_event_id, -year) |>
#   ungroup() |>
#   distinct()
#
# join <- full_join(comb, hbll2[, c("offset_hbll", "catch_count_hbll", "grouping_desc")]) # first join by depth
# join <- full_join(join, dog2[, c("offset_dog", "catch_count_dog", "grouping_desc")]) |>
#   distinct() # make NAs zero or keep NA if no fishing happened
# join <- full_join(join, dog2)
# join <- full_join(join, hbll2)
# join2 <- join |> drop_na(catch_count_hbll, catch_count_dog) |> distinct() # drop nas in the toal catch count column as that means those sites weren't sampled and therefore there is no pairwise comp
# join2[is.na(join2)] <- 0
# join2 <- left_join(join2, latlon) |> mutate(offset = offset_hbll - offset_dog)
#
# final_ld <- join2 |> mutate(cpue_hbll_length_depth = count_hbll_ld / exp(offset), cpue_dog_length_depth = count_dog_ld / exp(offset))
# final_ld <- final_ld |> drop_na()
# weight <- exp(final_ld$offset)
# dummy_mesh <- sdmTMB::make_mesh(final_ld, c("UTM.lon", "UTM.lat"), n_knots = 10)
#
# m2 <- sdmTMB(
#   # cbind(catch_count_hbll_length, catch_count_dog_length) ~ 1 + (1 | grouping_desc),
#   # cbind(catch_count_hbll_length, catch_count_dog_length) ~ 0 + factor(length_bin) + (1 | grouping_desc),
